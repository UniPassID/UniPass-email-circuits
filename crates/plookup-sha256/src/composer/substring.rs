...
impl<F: Field> Composer<F> {
    ...
    /// return Words(32bits) vec of "a_bits_location", "b_bits_location".
    ...
    pub fn add_substring_mask_poly_return_words(
        ...
        let mut output_words_a = vec![];
        for i in 0..max_lens / 32 {
            let mut tmp_vars = vec![];
            for j in 0..8 {
                let tmp = bit_location_a_value[32 * i + 4 * j] * F::from(8u64)
                    + bit_location_a_value[32 * i + 4 * j + 1] * F::from(4u64)
                    + bit_location_a_value[32 * i + 4 * j + 2] * F::from(2u64)
                    + bit_location_a_value[32 * i + 4 * j + 3];
                tmp_vars.push(self.alloc(tmp));
                self.poly_gate(
                    vec![
                        (tmp_vars[j], -F::one()),
                        (bit_location_a[32 * i + 4 * j], F::from(8u64)),
                        (bit_location_a[32 * i + 4 * j + 1], F::from(4u64)),
                        (bit_location_a[32 * i + 4 * j + 2], F::from(2u64)),
                        (bit_location_a[32 * i + 4 * j + 3], F::one()),
                    ],
                    F::zero(),
                    F::zero(),
                );
        ...
        let mut output_words_b = vec![];
        for i in 0..b_max_lens / 32 {
            let mut tmp_vars = vec![];
            for j in 0..8 {
                let tmp = bit_location_b_value[32 * i + 4 * j] * F::from(8u64)
                    + bit_location_b_value[32 * i + 4 * j + 1] * F::from(4u64)
                    + bit_location_b_value[32 * i + 4 * j + 2] * F::from(2u64)
                    + bit_location_b_value[32 * i + 4 * j + 3];
                tmp_vars.push(self.alloc(tmp));
                self.poly_gate(
                    vec![
                        (tmp_vars[j], -F::one()),
                        (bit_location_b[32 * i + 4 * j], F::from(8u64)),
                        (bit_location_b[32 * i + 4 * j + 1], F::from(4u64)),
                        (bit_location_b[32 * i + 4 * j + 2], F::from(2u64)),
                        (bit_location_b[32 * i + 4 * j + 3], F::one()),
                    ],
                    F::zero(),
                    F::zero(),
                );
