...
pub struct Email2048TripleCircuitInput {
    pub first_email_header_bytes: Vec<u8>,
    pub first_email_addr_pepper_bytes: Vec<u8>,
    pub first_email_header_pub_match: Vec<u8>,
    pub first_from_left_index: u32,
    pub first_from_len: u32,

    pub second_email_header_bytes: Vec<u8>,
    pub second_email_addr_pepper_bytes: Vec<u8>,
    pub second_email_header_pub_match: Vec<u8>,
    pub second_from_left_index: u32,
    pub second_from_len: u32,

    pub third_email_header_bytes: Vec<u8>,
    pub third_email_addr_pepper_bytes: Vec<u8>,
    pub third_email_header_pub_match: Vec<u8>,
...
impl Email2048TripleCircuitInput {
    ...
    pub fn new(private_inputs: Vec<PrivateInputs>) -> Result<Self, ProverError> {
        ...
        Ok(Self {
            first_email_header_bytes: all_email_header_bytes[0].clone(),
            first_email_addr_pepper_bytes: all_email_addr_pepper_bytes[0].clone(),
            first_email_header_pub_match: all_email_header_pub_match[0].clone(),
            first_from_left_index: all_from_left_index[0],
            first_from_len: all_from_len[0],
            second_email_header_bytes: all_email_header_bytes[1].clone(),
            second_email_addr_pepper_bytes: all_email_addr_pepper_bytes[1].clone(),
            second_email_header_pub_match: all_email_header_pub_match[1].clone(),
            second_from_left_index: all_from_left_index[1],
            second_from_len: all_from_len[1],
            third_email_header_bytes: all_email_header_bytes[2].clone(),
            third_email_addr_pepper_bytes: all_email_addr_pepper_bytes[2].clone(),
            third_email_header_pub_match: all_email_header_pub_match[2].clone(),
            third_from_left_index: all_from_left_index[2],
            third_from_len: all_from_len[2],
        })
    }
    ...
    pub fn synthesize(&self) -> Composer<Fr> {
        ...
        // collect pub_inputs
        // hash all public. 57504bits (256|256|2048|192|2048*8|16|16)*3
        // cal sha256( (a_hash|b_hash|a_bits_location|b_bits_location|pub_string|header_len|addr_len) * 3 )
        let mut sha256_all_public_data = vec![];

        // first email
        {
            // padding bytes
            let email_header_bytes_padding = padding_bytes(&self.first_email_header_bytes);
            let email_addr_pepper_bytes_padding =
                padding_bytes(&self.first_email_addr_pepper_bytes);
            let email_header_pub_match_padding = padding_bytes(&self.first_email_header_pub_match);
            ...
            let l = cs.alloc(Fr::from(self.first_from_left_index));
            // length of the email address
            let m = cs.alloc(Fr::from(self.first_from_len));
        ...
        {
            // padding bytes
            let email_header_bytes_padding = padding_bytes(&self.second_email_header_bytes);
            let email_addr_pepper_bytes_padding =
                padding_bytes(&self.second_email_addr_pepper_bytes);
            let email_header_pub_match_padding = padding_bytes(&self.second_email_header_pub_match);
            let from_padding_len = (email_addr_pepper_bytes_padding.len() / 64) as u32;
            let header_padding_len = (email_header_bytes_padding.len() / 64) as u32;
            ...
            // start index of the email address
            let l = cs.alloc(Fr::from(self.second_from_left_index));
            // length of the email address
            let m = cs.alloc(Fr::from(self.second_from_len));
        ...
        {
            // padding bytes
            let email_header_bytes_padding = padding_bytes(&self.third_email_header_bytes);
            let email_addr_pepper_bytes_padding =
                padding_bytes(&self.third_email_addr_pepper_bytes);
            let email_header_pub_match_padding = padding_bytes(&self.third_email_header_pub_match);
            let from_padding_len = (email_addr_pepper_bytes_padding.len() / 64) as u32;
            let header_padding_len = (email_header_bytes_padding.len() / 64) as u32;
            ...
            // start index of the email address
            let l = cs.alloc(Fr::from(self.third_from_left_index));
            // length of the email address
            let m = cs.alloc(Fr::from(self.third_from_len));
        ...
        // padding (288bits + 64bits)
        {
            ...
            let pad_value = Fr::from(57504u64);
            let tmp_var = cs.alloc(pad_value);
            cs.enforce_constant(tmp_var, pad_value);
            let word = Sha256Word {
                var: tmp_var,
        ...
        let all_public_hash =
            sha256_no_padding_words_var_fixed_length(&mut cs, &sha256_all_public_data, 113)
                .unwrap();
