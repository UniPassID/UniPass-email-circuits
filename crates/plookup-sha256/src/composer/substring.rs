...
impl<F: Field> Composer<F> {
    ...
    /// b is a public string
    pub fn hash_public_string_mimc(&mut self, b: &Vec<Variable>) -> Result<Variable, Error> {
        let max_lens = 2048;
        assert_eq!(b.len(), max_lens);

        // hash b to compress public_input
        // first collect chars 2048 = 66 * 31 + 2
        let mut mimc_data = vec![];
        let b_values = self.get_assignments(&b);
        for i in 0..66 {
            let mut tmp_vars = vec![];
            for j in 0..7 {
                let tmp = b_values[31 * i + 4 * j] * F::from(1u64 << 24)
                    + b_values[31 * i + 4 * j + 1] * F::from(1u64 << 16)
                    + b_values[31 * i + 4 * j + 2] * F::from(1u64 << 8)
                    + b_values[31 * i + 4 * j + 3];
                tmp_vars.push(self.alloc(tmp));
                self.poly_gate(
                    vec![
                        (tmp_vars[j], -F::one()),
                        (b[31 * i + 4 * j], F::from(1u64 << 24)),
                        (b[31 * i + 4 * j + 1], F::from(1u64 << 16)),
                        (b[31 * i + 4 * j + 2], F::from(1u64 << 8)),
                        (b[31 * i + 4 * j + 3], F::one()),
                    ],
                    F::zero(),
                    F::zero(),
            ...
            let tmp = b_values[31 * i + 28] * F::from(1u64 << 16)
                + b_values[31 * i + 29] * F::from(1u64 << 8)
                + b_values[31 * i + 30];
            tmp_vars.push(self.alloc(tmp));
            self.poly_gate(
                vec![
                    (tmp_vars[7], -F::one()),
                    (b[31 * i + 28], F::from(1u64 << 16)),
                    (b[31 * i + 29], F::from(1u64 << 8)),
                    (b[31 * i + 30], F::one()),
                ],
                F::zero(),
                F::zero(),
            ...
            let tmp_vars_values = self.get_assignments(&tmp_vars);
            let mut tmp2_vars = vec![];
            let tmp = tmp_vars_values[0] * F::from(1u128 << 96)
                + tmp_vars_values[1] * F::from(1u128 << 64)
                + tmp_vars_values[2] * F::from(1u64 << 32)
                + tmp_vars_values[3];
            tmp2_vars.push(self.alloc(tmp));
            self.poly_gate(
                vec![
                    (tmp2_vars[0], -F::one()),
                    (tmp_vars[0], F::from(1u128 << 96)),
                    (tmp_vars[1], F::from(1u128 << 64)),
                    (tmp_vars[2], F::from(1u64 << 32)),
                    (tmp_vars[3], F::one()),
                ],
                F::zero(),
                F::zero(),
            ...
            let tmp = tmp_vars_values[4] * F::from(1u128 << 88)
                + tmp_vars_values[5] * F::from(1u64 << 56)
                + tmp_vars_values[6] * F::from(1u64 << 24)
                + tmp_vars_values[7];
            tmp2_vars.push(self.alloc(tmp));
            self.poly_gate(
                vec![
                    (tmp2_vars[1], -F::one()),
                    (tmp_vars[4], F::from(1u128 << 88)),
                    (tmp_vars[5], F::from(1u64 << 56)),
                    (tmp_vars[6], F::from(1u64 << 24)),
                    (tmp_vars[7], F::one()),
                ],
                F::zero(),
                F::zero(),
            ...
            let tmp2_vars_values = self.get_assignments(&tmp2_vars);
            let tmp = tmp2_vars_values[0] * F::from(1u128 << 120) + tmp2_vars_values[1];
            mimc_data.push(self.alloc(tmp));
            self.poly_gate(
                vec![
                    (mimc_data[i], -F::one()),
                    (tmp2_vars[0], F::from(1u128 << 120)),
                    (tmp2_vars[1], F::one()),
                ],
                F::zero(),
                F::zero(),
        ...
        // 2 chars left
        let tmp = b_values[2046] * F::from(1u128 << 8) + b_values[2047];
        mimc_data.push(self.alloc(tmp));
        self.poly_gate(
            vec![
                (mimc_data[66], -F::one()),
                (b[2046], F::from(1u128 << 8)),
                (b[2047], F::one()),
            ],
            F::zero(),
            F::zero(),
        ...
        // mimc hash
        let hash = self.MiMC_sponge(&mimc_data, 1);
